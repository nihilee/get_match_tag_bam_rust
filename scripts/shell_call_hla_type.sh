...
if [ $insertCalc == 1 ]; then
	...
	echo -n -e "have copy the results into the outpath\n"
...
# /home/polysolver/binaries/sambamba view -t 4 $bam | grep -F -f $tag_file >> $outDir/tag.sam

# /home/polysolver/binaries/samtools view -bS -o $outDir/tag.bam $outDir/tag.sam

/home/polysolver/binaries/get_match_tag_bam $tag_file $bam $outDir/tag.bam
...
if [ $build == "hg38" ]; then
        ...
        $SAMTOOLS_DIR/samtools view $bam chr6:29941260-29945884 >> $outDir/chr6region.sam

        $SAMTOOLS_DIR/samtools view $bam chr6:31353872-31357187 >> $outDir/chr6region.sam

        $SAMTOOLS_DIR/samtools view $bam chr6:31268749-31272105 >> $outDir/chr6region.sam
...
java -jar /home/polysolver/binaries/SamToFastq.jar I=$outDir/chr6region.bam F=$outDir/chr6region.1.fastq F2=$outDir/chr6region.2.fastq VALIDATION_STRINGENCY=SILENT

perl /home/polysolver/scripts/clean_unpaired_fastq.pl $outDir/chr6region.1.fastq

perl /home/polysolver/scripts/clean_unpaired_fastq.pl $outDir/chr6region.2.fastq

# merge the two sets of fastqs

echo -n -e "** merge the two sets of fastqs\n"

perl /home/polysolver/scripts/merge_fastq.pl $outDir/tag $outDir/chr6region $outDir/merged
...
perl /home/polysolver/scripts/align_fork_fh.pl $outDir/merged.1.fastq $outDir/merged.2.fastq 20 $format /home/polysolver/data/abc_complete.nix $outDir/nv.complete.chr6region.R0k6.sam 0 /home/polysolver/binaries
...
java -jar /home/polysolver/binaries/SortSam.jar I=$outDir/nv.complete.chr6region.R0k6.bam O=$outDir/nv.complete.chr6region.R0k6.csorted.bam SORT_ORDER=coordinate TMP_DIR=/home/polysolver
...
# 这里调用了另一个shell脚本: shell_first_allele_calculations
perl /home/polysolver/scripts/first_allele_calculations_fork.pl $ids /home/polysolver samtools 20 $race $iFile $outDir
...
perl /home/polysolver/scripts/second_allele_calculations.pl $race $outDir/counts1.R0k6 $ids 1 /home/polysolver $outDir
...
# mkdir -p /tmp/rsync
# cp $outDir/{counts1.R0k6,counts2.R0k6,winners.hla.txt} $outDir/..
# rsync --delete-before -d /tmp/rsync/ $outDir/